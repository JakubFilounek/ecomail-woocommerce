...
use EcomailDeps\Monolog\Handler\SyslogUdp\UdpSocket;
/**
 ...
 */
class SyslogUdpHandler extends \EcomailDeps\Monolog\Handler\AbstractSyslogHandler
{
    const RFC3164 = 0;
    const RFC5424 = 1;
    const RFC5424e = 2;
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \EcomailDeps\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \EcomailDeps\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
     ...
     */
    public function setSocket(\EcomailDeps\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
