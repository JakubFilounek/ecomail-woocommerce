 ...
 */
class PushoverHandler extends \EcomailDeps\Monolog\Handler\SocketHandler
{
    private $token;
    private $users;
    private $title;
     ...
     */
    public function __construct(string $token, $users, ?string $title = null, $level = \EcomailDeps\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useSSL = \true, $highPriorityLevel = \EcomailDeps\Monolog\Logger::CRITICAL, $emergencyLevel = \EcomailDeps\Monolog\Logger::EMERGENCY, int $retry = 30, int $expire = 25200)
    {
        ...
        $this->highPriorityLevel = \EcomailDeps\Monolog\Logger::toMonologLevel($highPriorityLevel);
        $this->emergencyLevel = \EcomailDeps\Monolog\Logger::toMonologLevel($emergencyLevel);
        $this->retry = $retry;
        $this->expire = $expire;
    }
    ...
    private function buildContent(array $record) : string
    {
        ...
        $message = \EcomailDeps\Monolog\Utils::substr($message, 0, $maxMessageLength);
    ...
    {
        $this->highPriorityLevel = \EcomailDeps\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    public function setEmergencyLevel($value) : self
    {
        $this->emergencyLevel = \EcomailDeps\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
