...
use EcomailDeps\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use EcomailDeps\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \EcomailDeps\Monolog\Handler\Handler implements \EcomailDeps\Monolog\Handler\ProcessableHandlerInterface, \EcomailDeps\Monolog\ResettableInterface, \EcomailDeps\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \EcomailDeps\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\EcomailDeps\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \EcomailDeps\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \EcomailDeps\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \EcomailDeps\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \EcomailDeps\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \EcomailDeps\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \EcomailDeps\Monolog\Handler\HandlerInterface) {
