...
use EcomailDeps\PhpConsole\Connector;
use EcomailDeps\PhpConsole\Handler as VendorPhpConsoleHandler;
use EcomailDeps\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \EcomailDeps\Monolog\Handler\AbstractProcessingHandler
{
    private $options = [
        'enabled' => \true,
     ...
     */
    public function __construct(array $options = [], ?\EcomailDeps\PhpConsole\Connector $connector = null, $level = \EcomailDeps\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('EcomailDeps\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\EcomailDeps\PhpConsole\Connector $connector = null) : \EcomailDeps\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \EcomailDeps\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \EcomailDeps\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\EcomailDeps\PhpConsole\Helper::isRegistered()) {
            \EcomailDeps\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \EcomailDeps\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \EcomailDeps\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \EcomailDeps\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \EcomailDeps\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \EcomailDeps\Monolog\Formatter\LineFormatter('%message%');
    }
}
