...
use EcomailDeps\Monolog\Formatter\ChromePHPFormatter;
 ...
 */
class ChromePHPHandler extends \EcomailDeps\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \EcomailDeps\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \EcomailDeps\Monolog\Utils::jsonEncode(self::$json, \EcomailDeps\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode(\utf8_encode($json));
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \EcomailDeps\Monolog\Logger::WARNING, 'level_name' => \EcomailDeps\Monolog\Logger::getLevelName(\EcomailDeps\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
            self::$json['rows'][\count(self::$json['rows']) - 1] = $this->getFormatter()->format($record);
            $json = \EcomailDeps\Monolog\Utils::jsonEncode(self::$json, null, \true);
            $data = \base64_encode(\utf8_encode($json));
        }
        if (\trim($data) !== '') {
