...
use EcomailDeps\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \EcomailDeps\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \EcomailDeps\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \EcomailDeps\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \EcomailDeps\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \EcomailDeps\Monolog\Formatter\FormatterInterface
    {
        return new \EcomailDeps\Monolog\Formatter\LogglyFormatter();
    }
}
