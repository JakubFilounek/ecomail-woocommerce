...
use DateTimeZone;
use EcomailDeps\Monolog\Handler\HandlerInterface;
use EcomailDeps\Psr\Log\LoggerInterface;
use EcomailDeps\Psr\Log\InvalidArgumentException;
use Throwable;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \EcomailDeps\Psr\Log\LoggerInterface, \EcomailDeps\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\EcomailDeps\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \EcomailDeps\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    public function addRecord(int $level, string $message, array $context = []) : bool
    {
        ...
        foreach ($this->handlers as $handler) {
            if (null === $record) {
                ...
                $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => new \EcomailDeps\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                try {
    ...
    public function reset() : void
    {
        ...
        foreach ($this->processors as $processor) {
            if ($processor instanceof \EcomailDeps\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \EcomailDeps\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        ...
        if (!\is_int($level)) {
            throw new \EcomailDeps\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
        return $level;
    }
    /**
