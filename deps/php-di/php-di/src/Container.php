...
use EcomailDeps\DI\Definition\Resolver\ResolverDispatcher;
use EcomailDeps\DI\Definition\Source\DefinitionArray;
use EcomailDeps\DI\Definition\Source\MutableDefinitionSource;
use EcomailDeps\DI\Definition\Source\ReflectionBasedAutowiring;
use EcomailDeps\DI\Definition\Source\SourceChain;
use EcomailDeps\DI\Definition\ValueDefinition;
use EcomailDeps\DI\Invoker\DefinitionParameterResolver;
use EcomailDeps\DI\Proxy\ProxyFactory;
use InvalidArgumentException;
use EcomailDeps\Invoker\Invoker;
use EcomailDeps\Invoker\InvokerInterface;
use EcomailDeps\Invoker\ParameterResolver\AssociativeArrayResolver;
use EcomailDeps\Invoker\ParameterResolver\Container\TypeHintContainerResolver;
 ...
 */
class Container implements \EcomailDeps\Psr\Container\ContainerInterface, \EcomailDeps\DI\FactoryInterface, \EcomailDeps\Invoker\InvokerInterface
{
    /**
     ...
     */
    public function __construct(\EcomailDeps\DI\Definition\Source\MutableDefinitionSource $definitionSource = null, \EcomailDeps\DI\Proxy\ProxyFactory $proxyFactory = null, \EcomailDeps\Psr\Container\ContainerInterface $wrapperContainer = null)
    {
        $this->delegateContainer = $wrapperContainer ?: $this;
        $this->definitionSource = $definitionSource ?: $this->createDefaultDefinitionSource();
        $this->proxyFactory = $proxyFactory ?: new \EcomailDeps\DI\Proxy\ProxyFactory(\false);
        $this->definitionResolver = new \EcomailDeps\DI\Definition\Resolver\ResolverDispatcher($this->delegateContainer, $this->proxyFactory);
        // Auto-register the container
        $this->resolvedEntries = [self::class => $this, \EcomailDeps\Psr\Container\ContainerInterface::class => $this->delegateContainer, \EcomailDeps\DI\FactoryInterface::class => $this, \EcomailDeps\Invoker\InvokerInterface::class => $this];
    }
    /**
    ...
    public function get($name)
    {
        ...
        if (!$definition) {
            throw new \EcomailDeps\DI\NotFoundException("No entry or class found for '{$name}'");
        }
    ...
    public function injectOn($instance)
    {
        ...
        if (!$objectDefinition instanceof \EcomailDeps\DI\Definition\ObjectDefinition) {
            return $instance;
        }
        $definition = new \EcomailDeps\DI\Definition\InstanceDefinition($instance, $objectDefinition);
    ...
    public function set(string $name, $value)
    {
        if ($value instanceof \EcomailDeps\DI\Definition\Helper\DefinitionHelper) {
            ...
            $value = new \EcomailDeps\DI\Definition\FactoryDefinition($name, $value);
        }
        if ($value instanceof \EcomailDeps\DI\Definition\ValueDefinition) {
            $this->resolvedEntries[$name] = $value->getValue();
        } elseif ($value instanceof \EcomailDeps\DI\Definition\Definition) {
            $value->setName($name);
     ...
     * @return mixed
     */
    private function resolveDefinition(\EcomailDeps\DI\Definition\Definition $definition, array $parameters = [])
    {
        $entryName = $definition->getName();
        // Check if we are already getting this entry -> circular dependency
        if (isset($this->entriesBeingResolved[$entryName])) {
            throw new \EcomailDeps\DI\DependencyException("Circular dependency detected while trying to resolve entry '{$entryName}'");
        }
    ...
    protected function setDefinition(string $name, \EcomailDeps\DI\Definition\Definition $definition)
    {
    ...
    private function getInvoker() : \EcomailDeps\Invoker\InvokerInterface
    {
        if (!$this->invoker) {
            $parameterResolver = new \EcomailDeps\Invoker\ParameterResolver\ResolverChain([new \EcomailDeps\DI\Invoker\DefinitionParameterResolver($this->definitionResolver), new \EcomailDeps\Invoker\ParameterResolver\NumericArrayResolver(), new \EcomailDeps\Invoker\ParameterResolver\AssociativeArrayResolver(), new \EcomailDeps\Invoker\ParameterResolver\DefaultValueResolver(), new \EcomailDeps\Invoker\ParameterResolver\Container\TypeHintContainerResolver($this->delegateContainer)]);
            $this->invoker = new \EcomailDeps\Invoker\Invoker($parameterResolver, $this);
        }
        return $this->invoker;
    }
    private function createDefaultDefinitionSource() : \EcomailDeps\DI\Definition\Source\SourceChain
    {
        $source = new \EcomailDeps\DI\Definition\Source\SourceChain([new \EcomailDeps\DI\Definition\Source\ReflectionBasedAutowiring()]);
        $source->setMutableDefinitionSource(new \EcomailDeps\DI\Definition\Source\DefinitionArray([], new \EcomailDeps\DI\Definition\Source\ReflectionBasedAutowiring()));
        return $source;
    }
}
