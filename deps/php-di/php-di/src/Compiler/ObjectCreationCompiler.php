...
class ObjectCreationCompiler
{
     ...
     */
    private $compiler;
    public function __construct(\EcomailDeps\DI\Compiler\Compiler $compiler)
    {
        $this->compiler = $compiler;
    }
    public function compile(\EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
        ...
        try {
            ...
            throw \EcomailDeps\DI\Definition\Exception\InvalidDefinition::create($definition, \sprintf('Entry "%s" cannot be compiled: %s', $definition->getName(), $e->getMessage()));
        }
        return \implode("\n        ", $code);
    }
    public function resolveParameters(\EcomailDeps\DI\Definition\ObjectDefinition\MethodInjection $definition = null, \ReflectionMethod $method = null) : array
    {
        $args = [];
        if (!$method) {
            return $args;
        ...
        foreach ($method->getParameters() as $index => $parameter) {
            if (\array_key_exists($index, $definitionParameters)) {
                ...
                throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition(\sprintf('Parameter $%s of %s has no value defined or guessable', $parameter->getName(), $this->getFunctionName($method)));
            }
            $args[] =& $value;
        }
        return $args;
    }
    private function compileLazyDefinition(\EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
...
PHP;
    ...
    private function getParameterDefaultValue(\ReflectionParameter $parameter, \ReflectionMethod $function)
    {
        try {
            ...
            throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition(\sprintf('The parameter "%s" of %s has no type defined or guessable. It has a default value, ' . 'but the default value can\'t be read through Reflection because it is a PHP internal class.', $parameter->getName(), $this->getFunctionName($function)));
        }
    }
    ...
    private function assertClassIsNotAnonymous(\EcomailDeps\DI\Definition\ObjectDefinition $definition)
    {
        if (\strpos($definition->getClassName(), '@') !== \false) {
            throw \EcomailDeps\DI\Definition\Exception\InvalidDefinition::create($definition, \sprintf('Entry "%s" cannot be compiled: anonymous classes cannot be compiled', $definition->getName()));
        }
    }
    private function assertClassIsInstantiable(\EcomailDeps\DI\Definition\ObjectDefinition $definition)
    {
        ...
        throw \EcomailDeps\DI\Definition\Exception\InvalidDefinition::create($definition, \sprintf($message, $definition->getName()));
    }
}
