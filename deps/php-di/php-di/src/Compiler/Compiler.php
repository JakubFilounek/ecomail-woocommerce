...
namespace EcomailDeps\DI\Compiler;
...
use EcomailDeps\Opis\Closure\SerializableClosure;
use function rename;
...
class Compiler
{
    ...
    public function __construct(\EcomailDeps\DI\Proxy\ProxyFactory $proxyFactory)
    {
        $this->proxyFactory = $proxyFactory;
    }
    public function getProxyFactory() : \EcomailDeps\DI\Proxy\ProxyFactory
    {
     ...
     */
    public function compile(\EcomailDeps\DI\Definition\Source\DefinitionSource $definitionSource, string $directory, string $className, string $parentClassName, bool $autowiringEnabled) : string
    {
    ...
    {
        $tmpFile = @\tempnam(\dirname($fileName), 'swap-compile');
        if ($tmpFile === \false) {
     ...
     */
    private function compileDefinition(string $entryName, \EcomailDeps\DI\Definition\Definition $definition) : string
    {
        // Generate a unique method name
        $methodName = 'get' . ++$this->methodMappingCounter;
        $this->entryToMethodMapping[$entryName] = $methodName;
        switch (\true) {
            case $definition instanceof \EcomailDeps\DI\Definition\ValueDefinition:
                $value = $definition->getValue();
                $code = 'return ' . $this->compileValue($value) . ';';
                break;
            case $definition instanceof \EcomailDeps\DI\Definition\Reference:
                ...
                break;
            case $definition instanceof \EcomailDeps\DI\Definition\StringDefinition:
...
PHP;
            ...
            case $definition instanceof \EcomailDeps\DI\Definition\ArrayDefinition:
                try {
                    ...
                    throw new \EcomailDeps\DI\DependencyException(\sprintf('Error while compiling %s. %s', $definition->getName(), $e->getMessage()), 0, $e);
                }
                break;
            case $definition instanceof \EcomailDeps\DI\Definition\ObjectDefinition:
                $compiler = new \EcomailDeps\DI\Compiler\ObjectCreationCompiler($this);
            ...
            case $definition instanceof \EcomailDeps\DI\Definition\FactoryDefinition:
                ...
                if ($isInvokableClass && !$this->autowiringEnabled) {
                    throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition(\sprintf('Entry "%s" cannot be compiled. Invokable classes cannot be automatically resolved if autowiring is disabled on the container, you need to enable autowiring or define the entry manually.', $entryName));
                }
    ...
    public function compileValue($value) : string
    {
        ...
        if ($errorMessage !== \true) {
            throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition($errorMessage);
        }
    ...
    private function isCompilable($value)
    {
        ...
        if ($value instanceof \EcomailDeps\DI\Definition\DecoratorDefinition) {
    ...
    {
        $wrapper = new \EcomailDeps\Opis\Closure\SerializableClosure($closure);
        $reflector = $wrapper->getReflector();
        if ($reflector->getUseVariables()) {
            throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition('Cannot compile closures which import variables using the `use` keyword');
        }
        if ($reflector->isBindingRequired() || $reflector->isScopeRequired()) {
            throw new \EcomailDeps\DI\Definition\Exception\InvalidDefinition('Cannot compile closures which use $this or self/static/parent references');
        }
