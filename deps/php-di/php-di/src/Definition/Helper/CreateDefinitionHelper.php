 ...
 */
class CreateDefinitionHelper implements \EcomailDeps\DI\Definition\Helper\DefinitionHelper
{
    const DEFINITION_CLASS = \EcomailDeps\DI\Definition\ObjectDefinition::class;
    /**
     * @var string|null
     */
    private $className;
    ...
    public function getDefinition(string $entryName) : \EcomailDeps\DI\Definition\Definition
    {
        ...
        if (!empty($this->constructor)) {
            ...
            $constructorInjection = \EcomailDeps\DI\Definition\ObjectDefinition\MethodInjection::constructor($parameters);
            $definition->setConstructorInjection($constructorInjection);
        }
        if (!empty($this->properties)) {
            foreach ($this->properties as $property => $value) {
                $definition->addPropertyInjection(new \EcomailDeps\DI\Definition\ObjectDefinition\PropertyInjection($property, $value));
            }
        }
        if (!empty($this->methods)) {
            foreach ($this->methods as $method => $calls) {
                foreach ($calls as $parameters) {
                    ...
                    $methodInjection = new \EcomailDeps\DI\Definition\ObjectDefinition\MethodInjection($method, $parameters);
     ...
     */
    private function fixParameters(\EcomailDeps\DI\Definition\ObjectDefinition $definition, string $method, array $parameters) : array
    {
        $fixedParameters = [];
        foreach ($parameters as $index => $parameter) {
            ...
            if (\is_string($index)) {
                ...
                try {
                    ...
                    throw \EcomailDeps\DI\Definition\Exception\InvalidDefinition::create($definition, \sprintf("Parameter with name '%s' could not be found. %s.", $index, $e->getMessage()));
                }
