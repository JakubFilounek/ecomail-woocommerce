...
namespace EcomailDeps\DI\Definition\Dumper;
...
class ObjectDefinitionDumper
{
     ...
     */
    public function dump(\EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
    ...
    private function dumpConstructor(string $className, \EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
        $str = '';
    ...
    private function dumpProperties(\EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
        $str = '';
        foreach ($definition->getPropertyInjections() as $propertyInjection) {
            $value = $propertyInjection->getValue();
            $valueStr = $value instanceof \EcomailDeps\DI\Definition\Definition ? (string) $value : \var_export($value, \true);
            $str .= \sprintf(\PHP_EOL . '    $%s = %s', $propertyInjection->getPropertyName(), $valueStr);
        }
        return $str;
    }
    private function dumpMethods(string $className, \EcomailDeps\DI\Definition\ObjectDefinition $definition) : string
    {
        $str = '';
    ...
    private function dumpMethodParameters(string $className, \EcomailDeps\DI\Definition\ObjectDefinition\MethodInjection $methodInjection) : string
    {
