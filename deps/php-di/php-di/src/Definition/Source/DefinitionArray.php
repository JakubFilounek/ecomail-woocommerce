 ...
 */
class DefinitionArray implements \EcomailDeps\DI\Definition\Source\DefinitionSource, \EcomailDeps\DI\Definition\Source\MutableDefinitionSource
{
    const WILDCARD = '*';
    /**
     ...
     */
    private $normalizer;
    public function __construct(array $definitions = [], \EcomailDeps\DI\Definition\Source\Autowiring $autowiring = null)
    {
        if (isset($definitions[0])) {
            throw new \Exception('The PHP-DI definition is not indexed by an entry name in the definition array');
        }
        $this->definitions = $definitions;
        $autowiring = $autowiring ?: new \EcomailDeps\DI\Definition\Source\NoAutowiring();
        $this->normalizer = new \EcomailDeps\DI\Definition\Source\DefinitionNormalizer($autowiring);
    }
    /**
