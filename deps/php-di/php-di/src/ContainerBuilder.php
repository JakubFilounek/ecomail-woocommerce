...
use EcomailDeps\DI\Compiler\Compiler;
use EcomailDeps\DI\Definition\Source\AnnotationBasedAutowiring;
use EcomailDeps\DI\Definition\Source\DefinitionArray;
use EcomailDeps\DI\Definition\Source\DefinitionFile;
use EcomailDeps\DI\Definition\Source\DefinitionSource;
use EcomailDeps\DI\Definition\Source\NoAutowiring;
use EcomailDeps\DI\Definition\Source\ReflectionBasedAutowiring;
use EcomailDeps\DI\Definition\Source\SourceCache;
...
class ContainerBuilder
{
     ...
     */
    public static function buildDevContainer() : \EcomailDeps\DI\Container
    {
        return new \EcomailDeps\DI\Container();
    }
    /**
     * @param string $containerClass Name of the container class, used to create the container.
     */
    public function __construct(string $containerClass = \EcomailDeps\DI\Container::class)
    {
    ...
    public function build()
    {
        ...
        if ($this->useAnnotations) {
            $autowiring = new \EcomailDeps\DI\Definition\Source\AnnotationBasedAutowiring($this->ignorePhpDocErrors);
            $sources[] = $autowiring;
        } elseif ($this->useAutowiring) {
            $autowiring = new \EcomailDeps\DI\Definition\Source\ReflectionBasedAutowiring();
            $sources[] = $autowiring;
        } else {
            $autowiring = new \EcomailDeps\DI\Definition\Source\NoAutowiring();
        }
        $sources = \array_map(function ($definitions) use($autowiring) {
            if (\is_string($definitions)) {
                // File
                return new \EcomailDeps\DI\Definition\Source\DefinitionFile($definitions, $autowiring);
            } elseif (\is_array($definitions)) {
                return new \EcomailDeps\DI\Definition\Source\DefinitionArray($definitions, $autowiring);
            }
            return $definitions;
        }, $sources);
        $source = new \EcomailDeps\DI\Definition\Source\SourceChain($sources);
        // Mutable definition source
        $source->setMutableDefinitionSource(new \EcomailDeps\DI\Definition\Source\DefinitionArray([], $autowiring));
        if ($this->sourceCache) {
            if (!\EcomailDeps\DI\Definition\Source\SourceCache::isSupported()) {
                throw new \Exception('APCu is not enabled, PHP-DI cannot use it as a cache');
            }
            // Wrap the source with the cache decorator
            $source = new \EcomailDeps\DI\Definition\Source\SourceCache($source, $this->sourceCacheNamespace);
        }
        $proxyFactory = new \EcomailDeps\DI\Proxy\ProxyFactory($this->writeProxiesToFile, $this->proxyDirectory);
        $this->locked = \true;
        $containerClass = $this->containerClass;
        if ($this->compileToDirectory) {
            $compiler = new \EcomailDeps\DI\Compiler\Compiler($proxyFactory);
     ...
     */
    public function enableCompilation(string $directory, string $containerClass = 'CompiledContainer', string $containerParentClass = \EcomailDeps\DI\CompiledContainer::class) : self
    {
     ...
     * @return $this
     */
    public function wrapContainer(\EcomailDeps\Psr\Container\ContainerInterface $otherContainer) : self
    {
        $this->ensureNotLocked();
    ...
    public function addDefinitions(...$definitions) : self
    {
        ...
        foreach ($definitions as $definition) {
            if (!\is_string($definition) && !\is_array($definition) && !$definition instanceof \EcomailDeps\DI\Definition\Source\DefinitionSource) {
