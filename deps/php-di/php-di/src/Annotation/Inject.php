...
namespace EcomailDeps\DI\Annotation;
    ...
    public function __construct(array $values)
    {
        ...
        if (\is_array($values)) {
            foreach ($values as $key => $value) {
                if (!\is_string($value)) {
                    throw new \EcomailDeps\DI\Definition\Exception\InvalidAnnotation(\sprintf('@Inject({"param" = "value"}) expects "value" to be a string, %s given.', \json_encode($value)));
                }
