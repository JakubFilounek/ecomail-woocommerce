...
class CallableResolver
{
    ...
    public function resolve($callable) : callable
    {
        ...
        if (!\is_callable($callable)) {
            throw \EcomailDeps\Invoker\Exception\NotCallableException::fromInvalidCallable($callable, \true);
        }
        return $callable;
    }
    /**
    ...
    private function resolveFromContainer($callable)
    {
        ...
        if (\is_array($callable) && \is_string($callable[0])) {
            try {
                ...
                throw new \EcomailDeps\Invoker\Exception\NotCallableException(\sprintf('Cannot call %s() on %s because it is not a class nor a valid container entry', $callable[1], $callable[0]));
            }
        }
