...
namespace EcomailDeps\PhpDocReader;

use EcomailDeps\PhpDocReader\PhpParser\UseStatementParser;
use ReflectionClass;
...
class PhpDocReader
{
    ...
    {
        $this->parser = new \EcomailDeps\PhpDocReader\PhpParser\UseStatementParser();
    ...
    private function readPropertyType(\ReflectionProperty $property, bool $allowPrimitiveTypes) : ?string
    {
        ...
        if ($type[0] !== '\\') {
            ...
            if (!$resolvedType && !$this->ignorePhpDocErrors) {
                throw new \EcomailDeps\PhpDocReader\AnnotationException(\sprintf('The @var annotation on %s::%s contains a non existent class "%s". ' . 'Did you maybe forget to add a "use" statement for this annotation?', $class->name, $property->getName(), $type));
            }
            $type = $resolvedType;
        }
        if (!$this->ignorePhpDocErrors && !$this->classExists($type)) {
            throw new \EcomailDeps\PhpDocReader\AnnotationException(\sprintf('The @var annotation on %s::%s contains a non existent class "%s"', $class->name, $property->getName(), $type));
        }
    ...
    private function readParameterClass(\ReflectionParameter $parameter, bool $allowPrimitiveTypes) : ?string
    {
        ...
        if ($type[0] !== '\\') {
            ...
            if (!$resolvedType && !$this->ignorePhpDocErrors) {
                throw new \EcomailDeps\PhpDocReader\AnnotationException(\sprintf('The @param annotation for parameter "%s" of %s::%s contains a non existent class "%s". ' . 'Did you maybe forget to add a "use" statement for this annotation?', $parameterName, $class->name, $method->name, $type));
            }
            $type = $resolvedType;
        }
        if (!$this->ignorePhpDocErrors && !$this->classExists($type)) {
            throw new \EcomailDeps\PhpDocReader\AnnotationException(\sprintf('The @param annotation for parameter "%s" of %s::%s contains a non existent class "%s"', $parameterName, $class->name, $method->name, $type));
        }
        // Remove the leading \ (FQN shouldn't contain it)
        $type = \is_string($type) ? \ltrim($type, '\\') : null;
        return $type;
