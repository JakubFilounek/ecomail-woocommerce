...
class SerializableClosure implements \Serializable
{
    ...
    public function getReflector()
    {
        if ($this->reflector === null) {
            $this->reflector = new \EcomailDeps\Opis\Closure\ReflectionClosure($this->closure);
            $this->code = null;
        }
    ...
    {
        \EcomailDeps\Opis\Closure\ClosureStream::register();
        if (static::$securityProvider !== null) {
            if ($data[0] !== '@') {
                throw new \EcomailDeps\Opis\Closure\SecurityException("The serialized closure is not signed. " . "Make sure you use a security provider for both serialization and unserialization.");
            }
            if ($data[1] !== '{') {
                $separator = \strpos($data, '.');
                if ($separator === \false) {
                    throw new \EcomailDeps\Opis\Closure\SecurityException('Invalid signed closure');
                }
            ...
            if (!\is_array($data) || !static::$securityProvider->verify($data)) {
                throw new \EcomailDeps\Opis\Closure\SecurityException("Your serialized closure might have been modified and it's unsafe to be unserialized. " . "Make sure you use the same security provider, with the same settings, " . "both for serialization and unserialization.");
            }
            $data = $data['closure'];
        } elseif ($data[0] === '@') {
            if ($data[1] !== '{') {
        ...
        $this->closure = (include \EcomailDeps\Opis\Closure\ClosureStream::STREAM_PROTO . '://' . $this->code['function']);
    ...
    {
        if (static::$context === null) {
            static::$context = new \EcomailDeps\Opis\Closure\ClosureContext();
        }
    ...
    {
        if (static::$securityProvider === null) {
            static::$securityProvider = new \EcomailDeps\Opis\Closure\SecurityProvider($secret);
        }
    }
    /**
     * @param ISecurityProvider $securityProvider
     */
    public static function addSecurityProvider(\EcomailDeps\Opis\Closure\ISecurityProvider $securityProvider)
    {
    ...
    {
        \EcomailDeps\Opis\Closure\ClosureStream::register();
        return include \EcomailDeps\Opis\Closure\ClosureStream::STREAM_PROTO . '://function(' . $args . '){' . $code . '};';
    }
    /**
    ...
    protected function mapPointers(&$data)
    {
        ...
        if ($data instanceof static) {
            ...
            foreach ($data as $key => &$value) {
                ...
                } elseif ($value instanceof \EcomailDeps\Opis\Closure\SelfReference && $value->hash === $this->code['self']) {
            ...
            $scope[$data] = \true;
            foreach ($data as $key => &$value) {
                if ($value instanceof \EcomailDeps\Opis\Closure\SelfReference && $value->hash === $this->code['self']) {
                    $data->{$key} =& $this->closure;
            ...
            do {
                ...
                foreach ($reflection->getProperties() as $property) {
                    ...
                    if ($item instanceof \EcomailDeps\Opis\Closure\SerializableClosure || $item instanceof \EcomailDeps\Opis\Closure\SelfReference && $item->hash === $this->code['self']) {
                        $this->code['objects'][] = array('instance' => $data, 'property' => $property, 'object' => $item instanceof \EcomailDeps\Opis\Closure\SelfReference ? $this : $item);
    ...
    protected function mapByReference(&$data)
    {
        if ($data instanceof \Closure) {
            if ($data === $this->closure) {
                $data = new \EcomailDeps\Opis\Closure\SelfReference($this->reference);
                return;
            }
            ...
            unset($value);
        } elseif (\is_object($data) && !$data instanceof \EcomailDeps\Opis\Closure\SerializableClosure) {
            if (isset($this->scope[$data])) {
                $data = $this->scope[$data];
                return;
            }
