...
class Ecomail
{
    const JSONObject = 'jsono', JSONArray = 'jsona', PlainText = 'plaintext';
    ...
    private function send($request, $data = \NULL, $method = \NULL)
    {
        ...
        \curl_setopt($ch, \CURLOPT_HTTPHEADER, array('key: ' . $this->key, 'Content-Type: application/json'));
        $output = \curl_exec($ch);
        // Check HTTP status code
        if (!\curl_errno($ch)) {
            $http_code = \curl_getinfo($ch, \CURLINFO_HTTP_CODE);
            if ($http_code < 200 || $http_code > 299) {
                return array('error' => $http_code);
            }
        }
        \curl_close($ch);
        switch ($this->response) {
            ...
            case self::JSONObject:
                if (\is_array(\json_decode($output, \true))) {
