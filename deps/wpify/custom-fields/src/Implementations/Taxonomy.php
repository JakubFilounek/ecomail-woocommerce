 ...
 */
final class Taxonomy extends \EcomailDeps\WpifyCustomFields\Implementations\AbstractPostImplementation
{
    /** @var int */
    private $term_id;
    /** @var string */
    ...
    public function __construct(array $args, \EcomailDeps\WpifyCustomFields\WpifyCustomFields $wcf)
    {
        ...
        $args = wp_parse_args($args, array('taxonomy' => null, 'items' => array(), 'term_id' => null));
    ...
    public function register_meta()
    {
        ...
        foreach ($items as $item) {
            register_term_meta($this->taxonomy, $item['id'], array('type' => $this->get_item_type($item), 'description' => $item['title'], 'single' => \true, 'default' => $item['default'] ?? null));
        }
    }
    public function get_items()
    {
        $items = apply_filters('wcf_taxonomy_items', $this->items, array('taxonomy' => $this->taxonomy));
    ...
    {
        return array('object_type' => 'taxonomy', 'taxonomy' => $this->taxonomy, 'items' => $this->get_items());
    }
    /**
     * @param WP_Term $term
     */
