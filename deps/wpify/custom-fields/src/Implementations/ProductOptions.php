 ...
 */
final class ProductOptions extends \EcomailDeps\WpifyCustomFields\Implementations\AbstractPostImplementation
{
    /** @var int */
    private $product_id;
    /** @var array */
    ...
    public function __construct(array $args, \EcomailDeps\WpifyCustomFields\WpifyCustomFields $wcf)
    {
         ...
         */
        $args = wp_parse_args($args, array('product_id' => null, 'tab' => array('id' => 'general', 'label' => null, 'priority' => 100, 'target' => null, 'class' => array()), 'items' => array()));
        $this->tab = $args['tab'];
    ...
    public function register_meta()
    {
        foreach ($this->get_items() as $item) {
            register_post_meta('product', $item['id'], array('type' => $this->get_item_type($item), 'description' => $item['title'], 'single' => \true, 'default' => $item['default'] ?? null));
        }
    }
    public function get_items()
    {
        $items = apply_filters('wcf_product_options_items', $this->items, array('tab' => $this->tab));
    ...
    public function woocommerce_product_data_tabs(array $tabs)
    {
        if (isset($tabs[$this->tab['id']])) {
            ...
            $this->is_new_tab = \true;
            $tabs[$this->tab['id']] = array('label' => $this->tab['label'], 'priority' => $this->tab['priority'], 'target' => $this->tab['target'], 'class' => $this->tab['class']);
        }
        return $tabs;
    }
    /**
