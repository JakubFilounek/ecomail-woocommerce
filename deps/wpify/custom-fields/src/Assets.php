...
final class Assets
{
    ...
    public function register_script(string $file, array $deps = array(), $in_footer = \false, $localize = array())
    {
        ...
        if (wp_register_script($data['handle'], $data['src'], \array_merge($deps, $data['deps']), $data['ver'], $in_footer) && !empty($localize)) {
            foreach ($localize as $variable => $value) {
                wp_add_inline_script($data['handle'], 'try { var ' . $variable . ' = ' . wp_json_encode($value) . '; } catch (e) { console.error(e); }', 'before');
            }
        }
        return $data['handle'];
    }
    ...
    private function get_manifest()
    {
        if (empty($this->manifest) && \file_exists($this->assets_path . '/assets-manifest.json')) {
            ...
            $json = \json_decode($data, \true);
            foreach ($json as $key => $filename) {
                $path = $this->assets_path . '/' . $filename;
                $pathinfo = \pathinfo($path);
                $item = array('path' => $path, 'src' => $this->path_to_url($path), 'handle' => 'wcf-' . $pathinfo['filename'] . '-' . $pathinfo['extension']);
                if (!empty($deps[$key])) {
    ...
    public function get_code_editor_settings()
    {
        if (empty($this->code_editor_settings)) {
            $modes = array('css' => 'application/x-httpd-php', 'diff' => 'diff', 'html' => 'htmlmixed', 'gfm' => 'gfm', 'javascript' => 'javascript', 'json' => 'application/json', 'jsonld' => 'application/ld+json', 'jsx' => 'jsx', 'markdown' => 'markdown', 'nginx' => 'nginx', 'php' => 'php', 'sql' => 'sql', 'xml' => 'xml', 'yaml' => 'yaml');
