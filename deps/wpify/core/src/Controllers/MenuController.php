...
namespace EcomailDeps\Wpify\Core\Controllers;

use EcomailDeps\Wpify\Core\Abstracts\AbstractController;
use EcomailDeps\Wpify\Core\Menu;
class MenuController extends \EcomailDeps\Wpify\Core\Abstracts\AbstractController
{
    private $items = array();
    public function get_menu($slug = 0, $options = array())
    {
        $menu = $this->plugin->create_component(\EcomailDeps\Wpify\Core\Menu::class, ['slug' => $slug, 'options' => $options]);
        $menu->init();
        $items = array();
