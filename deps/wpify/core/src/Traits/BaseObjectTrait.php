...
use ReflectionClass;
use EcomailDeps\Wpify\Core\Exceptions\InexistentPropertyException;
use EcomailDeps\Wpify\Core\Exceptions\ReadOnlyException;
trait BaseObjectTrait
{
    ...
    public function __set($name, $value)
    {
        ...
        if (\method_exists($this, $func) || \method_exists($this, 'get' . \ucfirst($name))) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\ReadOnlyException(\sprintf('Property %s is read-only', $name));
        }
        $func = "is_{$name}";
        ...
        throw new \EcomailDeps\Wpify\Core\Exceptions\InexistentPropertyException(\sprintf('Inexistent property: %s', $name));
    }
    /**
     * @param $name
     *
