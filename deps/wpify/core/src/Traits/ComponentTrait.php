...
use EcomailDeps\Wpify\Core\Exceptions\ComponentInitFailureException;
use EcomailDeps\Wpify\Core\Exceptions\ComponentMissingException;
...
trait ComponentTrait
{
    ...
    public function get_plugin()
    {
        ...
        if ($this->plugin === $this && !$this instanceof \EcomailDeps\Wpify\Core\Abstracts\AbstractPlugin) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\PluginException(\sprintf('Plugin property on %s is equal to self. Did you forget to set the parent or create a getter?', $this->get_full_class_name()));
        }
        if (!$this->plugin instanceof \EcomailDeps\Wpify\Core\Abstracts\AbstractPlugin) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\PluginException(\sprintf('Parent property on %s not set. Did you forget to set the parent?', $this->get_full_class_name()));
        }
        return $this->plugin;
    }
    /**
    ...
    protected function try_init($component, $error = null)
    {
        ...
        if ($this->is_error($result)) {
            ...
            return new \EcomailDeps\Wpify\Core\Exceptions\ComponentInitFailureException(\vsprintf($message, $args));
        }
    }
    /**
    ...
    protected function load($component, ...$args)
    {
        ...
        foreach ($class as $index => $class_element) {
            ...
            if (!\class_exists($class_element)) {
                throw new \EcomailDeps\Wpify\Core\Exceptions\ComponentMissingException(\sprintf('Can not find class "%s" for Component "%s" in parent Component "%s"', $class_element, $component, __CLASS__));
            }
