...
use EcomailDeps\Wpify\Core\Models\WooOrderItemModel;
use EcomailDeps\Wpify\Core\Models\WooOrderItemProductModel;
use EcomailDeps\Wpify\Core\Models\WooOrderItemShippingModel;
/**
 * @package Wpify\Core
 */
abstract class AbstractWooOrderModel extends \EcomailDeps\Wpify\Core\Abstracts\AbstractComponent implements \EcomailDeps\Wpify\Core\Interfaces\PostTypeModelInterface
{
    /**
    ...
    public function get_line_items() : array
    {
        ...
        foreach ($this->wc_order->get_items() as $key => $item) {
            ...
            $order_item = $this->plugin->create_component(\EcomailDeps\Wpify\Core\Models\WooOrderItemProductModel::class, ['item' => $item]);
            $order_item->init();
            $items[] = $order_item;
        }
        return $items;
    ...
    public function get_shipping_items() : array
    {
        ...
        foreach ($this->get_wc_order()->get_items('shipping') as $key => $item) {
            ...
            $order_item = $this->plugin->create_component(\EcomailDeps\Wpify\Core\Models\WooOrderItemShippingModel::class, ['item' => $item]);
            $order_item->init();
            $items[] = $order_item;
        }
        return $items;
    ...
    public function get_fee_items() : array
    {
        ...
        foreach ($this->get_wc_order()->get_items('fee') as $key => $item) {
            /** @var WooOrderItemModel $order_item */
            $order_item = $this->plugin->create_component(\EcomailDeps\Wpify\Core\Models\WooOrderItemModel::class, ['item' => $item]);
            $order_item->init();
            $items[] = $order_item;
        }
        return $items;
