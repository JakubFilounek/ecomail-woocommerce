 ...
 */
abstract class AbstractPlugin extends \EcomailDeps\Wpify\Core\Abstracts\AbstractComponent
{
    /**
    ...
    protected function set_container()
    {
        ...
        if (!empty($namespace)) {
            if ('\\' !== $namespace[0]) {
                throw new \EcomailDeps\Wpify\Core\Exceptions\ContainerNotExistsException(\sprintf('Container namespace for Plugin %s must start with a backslash.', $this->get_full_class_name()));
            }
            if ('\\' === $namespace[\strlen($namespace) - 1]) {
                throw new \EcomailDeps\Wpify\Core\Exceptions\ContainerNotExistsException(\sprintf('Container namespace for Plugin %s must not end with a backslash.', $this->get_full_class_name()));
            }
        ...
        if (!\function_exists($container)) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\ContainerNotExistsException(\sprintf('Container function %s does not exist.', $container));
        }
        $this->container = $container();
        if (!$this->container instanceof \EcomailDeps\DI\Container) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\ContainerInvalidException(\sprintf('Container function %s does not return a Dice instance.', $container));
        }
    }
    /**
     * Plugin setup
