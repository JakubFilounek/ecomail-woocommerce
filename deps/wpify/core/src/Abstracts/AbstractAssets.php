 ...
 */
abstract class AbstractAssets extends \EcomailDeps\Wpify\Core\Abstracts\AbstractComponent
{
    /**
    ...
    public function add_asset(array $asset)
    {
        ...
        if (empty($asset['handle'])) {
            throw new \EcomailDeps\Wpify\Core\Exceptions\PluginException("Asset args have to contain 'handle'.");
        }
        if (empty($asset['file'])) {
