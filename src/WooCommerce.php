...
class WooCommerce extends AbstractComponent {
	/**
	 * @var EcomailApi
	 */
	private $ecomail_api;
	...
	public function __construct( EcomailApi $ecomail_api, WooOrderRepository $order_repository, Settings $settings ) {
	...
	public function setup() {
		...
		if ( $this->settings->get_setting( 'woocommerce_cart_tracking' ) ) {
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'set_cart_tracking_data' ) );
		}
	}
	...
	public function order_created( $order_id ) {
		if ( $this->settings->get_setting( 'woocommerce_checkout_subscribe' ) ) {
			if ( ! $this->settings->get_setting( 'woocommerce_checkout_subscribe_checkbox' ) ||
		...
		if ( $this->settings->get_setting( 'woocommerce_order_tracking' ) ) {
	 ...
	 * @param $order_id
	 *
	 * @throws \EcomailDeps\Wpify\Core\Exceptions\PluginException
	 */
	public function subscribe_contact( $order_id ) {
		...
		$data = array(
			...
			'update_existing'        => boolval( $this->settings->get_setting( 'woocommerce_checkout_update' ) ),
			'skip_confirmation'      => boolval( $this->settings->get_setting( 'woocommerce_checkout_skip_confirmation' ) ),
			'trigger_autoresponders' => boolval( $this->settings->get_setting( 'woocommerce_checkout_trigger_autoresponders' ) ),
		);

		$this->ecomail_api->add_subscriber( $this->settings->get_setting( 'woocommerce_checkout_list_id' ), $data );
	}
	...
	public function set_cart_tracking_data() {
		if ( ! WC()->session->get( 'ecomail_update_cart' ) ) {
			return;
		}
	...
	public function track_cart( $items, $email ) {
		...
		foreach ( $items as $item ) {
			...
			$products[] = array(
				'productId'   => $item,
				'img_url'     => wp_get_attachment_image_url( $prod->get_image_id() ),
	...
	public function add_checkbox() {
		if ( ! $this->settings->get_setting( 'woocommerce_checkout_subscribe' ) || ! $this->settings->get_setting( 'woocommerce_checkout_subscribe_checkbox' ) ) {
			return;
		}
		?>
		<p class="form-row">
			<label class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox">
				...
				<?php
					echo sanitize_text_field( $this->settings->get_setting( 'woocommerce_checkout_subscribe_text' ) );
				?>
					</span>&nbsp
			</label>
		</p>
